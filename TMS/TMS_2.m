...
VRT = 20;                            % коэффициент перед 't' по варианту
Am = 0.5;                             % коэффициент перед 'sin' или 'cos' по варианту

Tc=N*Ts;
L=(To/Ts);

L=32;       % почему-то работает только при определенных значениях (методом тыка)
...
Sz(N/2 + 1 :N)=0;       % обнуляем часть спектра с отрицательной частотой
...
figure
plot(Uam, '--r', 'LineWidth', 3)
hold on
grid on
plot(Ussb, '-b', 'LineWidth', 3)
legend('Uam', 'Ussb')

figure
plot(TETA, '--r', 'LineWidth', 3)
hold on
grid on
plot(OMbf, '-b', 'LineWidth', 3)
legend('TETA', 'OMbf')
...
legend('Spectrum (SSB-signal)', 'Spectrum (analytic signal)')
axis( [ N/2-N/8 N/2+N/8 0 max(abs(Sz))] )
...
TET=TET+ K/N*abs(min(TET));     % костыли, чтобы график ТЕТА получше выглядел
